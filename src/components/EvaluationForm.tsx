...
import { PaperclipIcon, UploadIcon, FileTextIcon, BookOpenIcon, ClipboardCheckIcon, AlertTriangle, RefreshCw, LayoutGrid, LayoutList } from "lucide-react";
  ...
  const handleEvaluate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result) {
        ...
        if (!result.marksAwarded || !result.performanceLabel || !result.feedbackSummary) {
          console.error("Evaluation result is missing required properties:", result);
        ...
        <div className={`${isMobile ? 'space-y-4' : 'space-y-5'}`}>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              className={`${isMobile ? 'min-h-[60px]' : 'min-h-[80px]'} focus:border-app-teal-300 ${questionText ? 'bg-gray-50' : ''}`}
              required
            />
            {questionText && (
