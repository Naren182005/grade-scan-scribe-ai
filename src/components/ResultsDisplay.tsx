...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({
  ...
  const percentage = totalMarks > 0
    ? Math.round((result.marksAwarded / totalMarks) * 100)
    : 0;
          ...
          <div className="bg-blue-50 p-4 rounded-md border border-blue-100">
            <h3 className="font-medium text-app-blue-800 mb-3 flex items-center">
              Performance: <span className={`ml-2 ${gradeColor} font-semibold`}>
                {percentage === 100 ? 'Good' : (result.performanceLabel || getPerformanceLabel(percentage))}
              </span>
            </h3>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {percentage === 100 ? (
                ...
                (result.feedbackSummary || []).map((feedback, idx) => (
